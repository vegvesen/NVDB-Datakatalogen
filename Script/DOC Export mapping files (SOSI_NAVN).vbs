...
sub recLoopSubPackages(p)
	...
	for each e in p.elements
	   If e.Stereotype="featureType" or e.Stereotype="dataType" then
			set tv=e.TaggedValues.GetByName("SOSI_navn")
			if not tv is nothing then
				objFtFile.Write tv.Value & ";" & e.Name & vbCrLf
				for each a in e.Attributes
					set atv=nothing
					On error resume next
					set atv=a.TaggedValues.GetByName("SOSI_navn")
...
sub exportMappingFiles()
	...
	if not thePackage is nothing and thePackage.ParentID <> 0 then
		...
		Set objFtFile = objFSO.CreateTextFile(path & "\" & name & "_ftSOSIMapping.csv",True)
		Set objPtFile = objFSO.CreateTextFile(path & "\" & name & "_ptSOSIMapping.csv",True)
		Set objEnFile = objFSO.CreateTextFile(path & "\" & name & "_enSOSIMapping.csv",True)
