				...
				for each con in el.Connectors
					if con.type = "Aggregation" or con.Type = "Association" then
						if con.SupplierID <> el.ElementID then
							...
							'Setter assosiasjonen som navigerbar
							...
							Repository.WriteOutput "Script", Now & " " & con.ClientEnd.Navigable & " assosiasjon til " & assEl.Name, 0 
						end if
						'Importer assosiert pakke - ikke Vegreferanse
						if assEl.Alias <> 532 then 'navigable = "Navigable" and 
						...
						if scSubPck.PackageGUID <> pkSOSIfelles.PackageGUID then
							...
							for each scEl In scSubPck.elements
								if scEl.Stereotype="featureType" then
									...
									'Legger kun til arv for hovedobjektpakken. De andre får arv i sine egne hovedpakker
