	...
	if not thePackage is nothing and thePackage.ParentID <> 0 then
		...
		for each pck in thePackage.Packages
			if pck.Alias <> 532 then 'Vegreferanse skal ikke være med
				...
				'Importerer navigerbart assosierte objekttyper
				...
				for each con in el.Connectors
					...
					if con.type = "Aggregation" or con.Type = "Association" then
						if con.SupplierID <> el.ElementID and con.SupplierEnd.Navigable = "Navigable" then
							set assEl = Repository.GetElementByID(con.SupplierID)
							navigable = true
						elseif con.ClientID <> el.ElementID and con.ClientEnd.Navigable = "Navigable" then
							set assEl = Repository.GetElementByID(con.ClientID)
							navigable = true
						end if
						Repository.WriteOutput "Script", Now & " " & con.SupplierEnd.Navigable & " assosiasjon til " & assEl.Name, 0 
						'Importer assosiert pakke - ikke Vegreferanse
						if navigable and assEl.Alias <> 532 then 
				...
				'Legger til arv fra SOSI Fellesegenskaper for hovedobjekttypen og rydder i tagger
						...
						if scSubPck.PackageGUID <> pkSOSIfelles.PackageGUID then
							...
							for each scEl In scSubPck.elements
								if scEl.Stereotype="featureType" then
									...
									'Legger kun til arv for hovedobjektpakken. De andre eventuelt får arv i sine egne hovedpakker
									'If scEl.Name = "Dokumentasjon" Or scEl.Name = "Kommentar" Or scEl.Name = "Systemobjekt" Or Mid(scEl.Name, 1, 8) = "Tilstand" Then
									If scEl.Alias <> pck.Alias or scEl.Name = "Dokumentasjon" Or scEl.Name = "Kommentar" Or scEl.Name = "Systemobjekt" Or Mid(scEl.Name, 1, 8) = "Tilstand" Then
